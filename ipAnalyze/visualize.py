...
def barplot_Overview(index=[1,3],name=None,Df=None):
    ...
    f, ax = plt.subplots(figsize=(13, len(data)))
    f.autofmt_xdate()
    ...
    for p in ax1.patches:
        ax.annotate('{:.3f}%'.format(p.get_width()*100/335230),
                    ...
                    fontsize=15,va='center',color="white")
    ...
    plt.title("Number of IPV4 by country",
                 fontsize=22,color="#e5fdff",fontname='Console')

    a=ax.legend(ncol=1, loc="upper left", frameon=True,fontsize=13,
    ...
    ax.tick_params(axis="y",labelsize=11,colors="w")

    plt.xlabel("Summary",fontsize=14,color="#51d0ff")
    plt.ylabel("Country",size=11,color="#51d0ff")
